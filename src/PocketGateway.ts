...
export class PocketGateway {
  ...
  public async getSavedItems(
    ...
    const results = await fetch("https://getpocket.com/v3/get", {
      ...
      body: JSON.stringify({
        ...
        count,
        sort: "oldest", // Sort by oldest so we add the items in the same order as they were saved.
        ...(since ? { since } : {}),
        ...(offset ? { offset } : {}),
      }),
    });
    ...
    if (data.error) {
      throw new Error(
        `Something went wrong with fetching the items: ${data.error}`
      );
    }
  ...
  public async addItem({ url }: { url: string }) {
    ...
    if (!results.ok) {
      throw new Error(
        `(${results.status}) Something went wrong with adding the item: ${results.body}`
      );
    }
